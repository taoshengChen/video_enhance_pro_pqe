...
3.  **肤色保护集成阶段:**
    ...
    *   一个调节因子 $P$（$P = 1 - S$，其中 $S$ 是来自“肤色保真”模块的肤色概率）被用于衰减 `amt_val`。这意味着，对于被识别为肤色的像素，饱和度增强效果将被减弱甚至完全抑制，从而保护肤色区域不产生失真。此调制过程由包括乘法器 **U8** 在内的逻辑处理，其中 `distance` 信号会用于实现更精细的非线性调节。
...
这两个查找表被存入两个单端口SRAM中（每 2 个LANE需要一个 SRAM），使得可以根据像素的`cr`和`cb`值在一个时钟周期内并行读出`f(cr)`和`g(cb)`。最后，通过一个加法器将这两个值相加（`dist = f(cr) + g(cb)`），即可高效地计算出最终的肤色概率。这种方法将复杂的乘法和除法运算转换为了两次内存读取和一次加法，极大地节约了逻辑资源。
